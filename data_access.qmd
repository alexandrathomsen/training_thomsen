...
delta_sites <- readr::read_csv(delta_sites_url, show_col_types=F)
head(delta_sites)
```
...
delta_sites <- readr::read_csv(delta_sites_pin, show_col_types=F)
head(delta_sites)
```
...
```{r}
delta_catch_file <- contentid::resolve(delta_catch_id, store=T)
delta_catch <- readr::read_csv(delta_catch_file, show_col_types=F)
head(delta_catch)
```
...
Taking content id and getting list of where it exists:
