...
head(ak_regions)
glimpse(ak_regions)
st_crs(ak_regions) # view coordinate reference system
```
...
## Join summarized dataset with city population geospatial dataset
```{r}
...
pop_mgmt_3338 <- pop_region_3338 %>% 
  group_by(mgmt_area) %>% 
  summarize(total_pop = sum(total_pop), do_union = FALSE) # keeps region boundaries
...
m <- leaflet(options = leafletOptions(crs = epsg3338)) %>% 
  addPolygons(data = pop_region_4326, fillColor = "gray",
              weight = 1)
m
```
...
m <- leaflet(options = leafletOptions(crs = epsg3338)) %>% 
  addPolygons(data = pop_region_4326, fillColor = ~ pal(total_pop),
              weight = 1,
              color = "black",
              fillOpacity = 1,
              label = ~ region) %>% 
  addCircleMarkers(data = pop_4326,
                   ...
                   weight = 0.25,
                   color = "black",
                   label = ~ paste0(pop_4326$city, ", population ",
                                    comma(pop_4326$population))) %>% 
